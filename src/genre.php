...
<div class="navbar flex justify-between bg-base-100 fixed top-0 left-0 w-full z-50 shadow-lg px-10">
    ...
    <div class="flex flex-col justify-start mt-20 w-full min-h-screen">
        ...
        <div class=" mx-10 bg-gray-800 rounded-xl mt-5 flex flex-col px-20" style="background-color:#808080">
            ...
            <?php while ($film_data = mysqli_fetch_assoc($genre_data)): ?> 
                <?php $film_id = $film_data['film_id'];?>
                <?php $film_names = mysqli_query($conn, "SELECT * FROM film WHERE film_id = $film_id ORDER BY judul ASC");?>
                <?php $film_name = mysqli_fetch_assoc($film_names);?>            

                <!--LOOP REKOMENDASI-->
                <div class="flex flex-col">
                    <div class="flex border-b border-gray-500">

                        <a href='detail.php?id=<?php echo $film_name["film_id"]?>'>
                            <img src="img/<?php echo $film_name['gambar']; ?>"alt=" " class="max-w-32  py-3  ">
                        </a>

                        <div class="flex flex-col p-2 ">
                            <a href='detail.php?id=<?php echo $film_name["film_id"]?>' class="font-bold text-xl"><?php echo $film_name['judul']?></a>
                            <div class="text-sm"><?php echo $film_name['tahun']?></div>
                            <div class="flex">
                                <img src="img/star.png" alt="star" class="h-6 w-auto mr-1">
                                <div><?php echo $film_name['rating']?></div>
                                <img src="img/mafilmstar.png" alt="star" class="h-6 w-auto mr-1 ml-3">
                                <div>
                                    <?php 
                                        if(isset($average_ratings[$film_name['film_id']])) {
                                            echo $average_ratings[$film_name['film_id']];
                                        } else {
                                            echo "0";
                                        }
                                    ?>
                            ...
                            <div class=""><?php echo $film_name['sinopsis']?></div>
                            <div class="py-5 flex">
                                <div class="mr-1">Directur</div>
                                <div class="mr-1 " style="color: cornflowerblue;"><?php echo $film_name['direktur']?></div>
                                <div class="mr-1">Pemeran</div>
                                <div class="mr-3" style="color: cornflowerblue;"><?php echo $film_name['pemeran']?></div>
